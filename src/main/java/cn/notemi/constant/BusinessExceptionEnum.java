...
public enum BusinessExceptionEnum {
	 ...
	 * 无效参数
	 */
	PARAMETER_INVALID(BusinessException.class, HttpStatus.BAD_REQUEST, ResultCode.PARAM_IS_INVALID),

	/**
	 * 数据未找到
	 */
	NOT_FOUND(BusinessException.class, HttpStatus.NOT_FOUND, ResultCode.RESULE_DATA_NONE),

	/**
	 * 接口方法不允许
	 */
	METHOD_NOT_ALLOWED(BusinessException.class, HttpStatus.METHOD_NOT_ALLOWED, ResultCode.INTERFACE_ADDRESS_INVALID),

	/**
	 * 数据已存在
	 */
	CONFLICT(BusinessException.class, HttpStatus.CONFLICT, ResultCode.DATA_ALREADY_EXISTED),

	/**
	 * 用户未登录
	 */
	UNAUTHORIZED(BusinessException.class, HttpStatus.UNAUTHORIZED, ResultCode.USER_NOT_LOGGED_IN),

	/**
	 * 无访问权限
	 */
	FORBIDDEN(BusinessException.class, HttpStatus.FORBIDDEN, ResultCode.PERMISSION_NO_ACCESS),

	/**
	 * 远程访问时错误
	 */
	REMOTE_ACCESS_ERROR(BusinessException.class, HttpStatus.INTERNAL_SERVER_ERROR, ResultCode.INTERFACE_OUTTER_INVOKE_ERROR),

	/**
	 * 系统内部错误
	 */
	INTERNAL_SERVER_ERROR(BusinessException.class, HttpStatus.INTERNAL_SERVER_ERROR, ResultCode.SYSTEM_INNER_ERROR);
